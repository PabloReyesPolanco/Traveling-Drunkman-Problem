...
class Instancia:
    def __init__(self, nombre):
        ...
        self.deposito = 0
        self.coordenadas = [
            tuple(map(float, archivo.readline().rstrip().split(" "))) for _ in range(self.n)]
        archivo.close()
        ...
        for i in range(n):
            for j in range(n):
                if i == j:
                    ...
                    aux = nx.shortest_path_length(
                        G, origen, destino, weight='length')
                    distancias[i, j] = aux
        self.G = G
    ...
    def resolver(self):
        ...
        arcos, costo = gp.multidict({(i, j): self.distancia(
        ...
        unaSalida = m.addConstrs(
            (x.sum(i, '*') == 1 for i in N), name='unaSalida')
        unaEntrada = m.addConstrs(
            (x.sum('*', j) == 1 for j in N), name='unaEntrada')
        deposito = m.addConstr((g.sum(0, '*') == 0), name='deposito')
        orden = m.addConstrs(
            (g.sum(i, '*') - g.sum('*', i) == 1 for i in N[1:]), name="orden")
        gmaximo = m.addConstrs((g[i, j] <= (n - 1) * x[i, j]
                               for i, j in arcos), name="gmaximo")
        gpositivo = m.addConstrs(
            (g[i, j] >= 0 for i, j in arcos), name="gpositivo")
        xpositivo = m.addConstrs(
            (g[i, j] >= 0 for i, j in arcos), name="xpositivo")
    ...
    def visualizarCiudad(self):
        ...
        G = self.G
        rutas = [ox.shortest_path(G, ox.get_nearest_node(G, self.coordenadas[i]), ox.get_nearest_node(
            G, self.coordenadas[j])) for i, j in arcosSolucion]
        fig, ax = ox.plot_graph_routes(
            self.G, rutas, "r", route_linewidth=6, node_size=0)
